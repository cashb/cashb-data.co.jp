<?php require "conf_cashb.php";	//キャッシュビー法人問合せ用 ?>
<?php require "mail_cashb.php";	//キャッシュビー法人問合せ用メール文面 ?>
<?php require "ml_skr_function.php";	//さくらメール用 ?>
...
//$to = "yamaguchi@lunion.jp";
$to = "hideyuki.ando@cashb.co.jp,support@cashb-data.co.jp";

//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
//$from = "webworks_developing@lunion.jp";
$from = "support@cashb-data.co.jp";
...
// $Referer_check_domain = "cashb.webworks.tokyo.jp";
$Referer_check_domain = "cashb-data.co.jp";
...
$subject = "【キャッシュビーデータお問い合わせ】サイトからのお問い合わせ";
...
$require = array('f-name','s-name','f-kana','s-kana','company','organ','tel','email','know','inq-naiyo','privacy');
...
$re_subject = "【キャッシュビーデータ株式会社】お問い合わせありがとうございました";
...
この度はキャッシュビーデータ株式会社へのお問い合わせありがとうございました。
以下の内容でお問い合わせを受付致しました。
折り返しご連絡致しますので今しばらくお待ちください。
...
2番7号 赤坂Kタワー4階
TEL:03-6890-3212
URL:https://www.cahsb-data.co.jp/
...
//----------------------------------------------------------------------
//  確認画面用変換テーブル
//  name属性に対し表示したい文字を設定する  nametojp
//  selectで選択されたものを表示したい文字を設定する　knowtojp
//----------------------------------------------------------------------
$nametoj_flg = 1;  //name属性を$nametojpに置き換える
$nametojp = array('f-name'=>'姓','s-name'=>'名','f-kana'=>'姓（カナ）','s-kana'=>'名（カナ）','company'=>'貴社名','organ'=>'部署名','title'=>'お役職','tel'=>'お電話番号','email'=>'メールアドレス','know'=>'当社を知ったきっかけ','privacy'=>'「個⼈情報のお取扱いに関する同意事項」に同意します','inq-naiyo'=>'お問合せ内容');
$knowtojp_flg = 0;  //selectのvalueをknowtojpに置き換える
$knowtojp = array(' ','当社を知ったきっかけ','エンジン','比較サイト・クチコミサイト','SNS・ブログ','インターネット広告','新聞・雑誌','ダイレクトメール','家族・友達・知人の紹介','キャッシュビーアプリを知っていた'.'その他');
...
//		mail($to,$subject,$adminBody,$header);
		skr_mailto($to,$subject,$adminBody,$from,$header);
		if($remail == 1 && !empty($post_mail)) skr_mailto($post_mail,$re_subject,$userBody,$from,$reheader);
	}else{
...
<head>
<meta charset="UTF-8" />
<title>お問い合わせ確認｜キャッシュビーデータ株式会社</title>
<meta name="description" content="キャッシュビーデータ株式会社　お問い合わせページ">
...
<!--css-->
<link rel="stylesheet" href="../assets/css/style.css?ver=1.2" media="all">
...
<!--favicon-->
  ...
  <script src="./js/media_formchk.js" type="text/javascript"></script>
...
})(window,document,'script','dataLayer','GTM-MJ8TJTQ');</script>
<!-- End Google Tag Manager -->

</head>
<body>
<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-MJ8TJTQ"
...
<!-- ▼************ 送信内容表示部　 ************ ▼-->
    ...
    <div class="wrap">
        <a href="/"><img class="logo" src="../assets/img/common/logo.png" alt="キャッシュビーデータ"></a>
        ...
        <nav>
            ...
            <div>
                <a class="btn" href="../contact" class="text-decoration-none">お問い合わせ</a>
                <a class="btn" href="../download" class="text-decoration-none">資料ダウンロード</a>
            </div>
        </nav>
    </div>
    <nav class="sp_menu">
        ...
        <div class="bnr_box">
            <a href="/contact/" class="btn text-decoration-none">お問い合わせ</a><a href="/download/" class="btn text-decoration-none">資料ダウンロード</a>
        </div>
    </nav>
    <div class="navToggle">
...
<?php }else{ ?>
    ...
    <div class="container">
		<div class="col-12 mb-5">
			<p>
			以下の内容でよろしければ、「送信する」ボタンをクリックしてください。<br>
※内容を修正する場合は、「戻る」ボタンをクリックし、修正してください。
			</p>
		</div>
    ...
    <div class="wrap">
        ...
        <div class="right">
            <nav>
                ...
                <a href="/contact/" class="text-decoration-none">お問い合わせ</a>
                <a href="/download/" class="text-decoration-none">資料ダウンロード</a>
            </nav>
        </div>
    </div>
    <div class="under">
        <div class="wrap">
            <div class="link"><a href="/privacy/" class="text-decoration-none">個人情報保護方針</a><a href="/careers/" class="text-decoration-none">採用情報</a></div>
            <p>&copy;︎2021 CASHb DATA,Inc.</p>
        </div>
    </div>
</footer>
...
/* ▼▼▼送信完了画面のレイアウト　編集可 ※送信完了後に指定のページに移動しない場合表示▼▼▼　*/
?>
<!DOCTYPE HTML>
<html lang="ja">
<head>
...
function postToMail($arr){
	...
	global $nametoj_flg,$nametojp;
	global $knowtojp_flg,$knowtojp;
	global $touser_str;
...
//			$resArray .= "【 ".h($key_jp)." 】 ".h($out_jp)."\n";

			$befor_str_val = '##val_'.$key;
			$after_str_val = $out_jp;

			$replace = [
    			$befor_str_val => $after_str_val,
				'<br />' => '',					// <br>削除
			];

		   $touser_str = strtr($touser_str, $replace);
...
//管理者用　送信メールにPOSTデータをセットする関数
function postToAdminMail($arr){
	global $hankaku,$hankaku_array;
	global $nametoj_flg,$nametojp;
	global $knowtojp_flg,$knowtojp;
	global $toadmin_str;
...
//			$resArray .= "【 ".h($key_jp)." 】 ".h($out_jp)."\n";
		   ...
		   $toadmin_str = strtr($toadmin_str, $replace);
		   
		   
		}
	}
	$resArray .= $toadmin_str;
...
//select要素変換
		...
		if($nametoj_flg == 1){
			$key_jp = $nametojp[$key];
//			$html .= "<tr><th>".$key_jp."</th><td>".$out_jp;
		$befor_str_key = '##key_'.$key;
		$after_str_key = $key_jp;
		$befor_str_val = '##val_'.$key;
		$after_str_val = $out_jp;

$replace = [
    $befor_str_key => $after_str_key,
    $befor_str_val => $after_str_val,
];
		   $con_html_str = strtr($con_html_str, $replace);
		   
		}
		else{
//			$html .= "<tr><th>".$key."</th><td>".$out_jp;
		$befor_str_key = '##key_'.$key;
		$after_str_key = $key;
		$befor_str_val = '##val_'.$key;
		$after_str_val = $out_jp;
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	$adminBody="「".$subject."」から以下のお問い合わせがありました。\n\n";
	$adminBody.="送信された日時：".date( "Y/m/d (D) H:i:s", time() )."\n";
	$adminBody .="＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
	$adminBody.= postToAdminMail($arr);//POSTデータを関数からセット
	$adminBody.="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";

//問い合わせのページURLはメールに記載しない。
//	if($confirmDsp != 1){
...
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="/">©︎2016 CASHb,Inc.</a>';
}
//----------------------------------------------------------------------
//  関数定義(END)
//----------------------------------------------------------------------
